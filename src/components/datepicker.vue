			...
			<div class="row" @click="show = !show">
				<div>
					<input style="width: 300px"
						...
						:value="value"
						>
					<div class="glyphicon glyphicon-calendar sm-col-3"></div>
				</div>
    ...
    export default {
        ...
        methods: {
			...
			nowParse () {
				...
				var tDate = null;
				var tYear = null;
				tempDate = store.state.score.storeTempDate
				tMonth = tempDate.getMonth();
				tDate = tempDate.getDate();
				tYear = tempDate.getFullYear();
				this.parsedDate = tMonth + "/" + tDate + "/" + tYear
				console.log("tempDate: " + parsedDate)
			},
            update () {
                var arr = [];
            ...
            pickDate (index) {
				...
				store.commit('updateDatePicker', this.now)
            },
            parse (str) {
        ...
        beforeMount () {
            this.now = this.parse(this.value) || this.parse(new Date);
