<template>
	<div>
		...
		<div v-for="score in updateStoreScoreboard" id="dailyScoreMain" class="container">
			<div v-if="!score.status" v-for="scores in updateNoScore">
				<p v-if="noScoreObj.rendered == false">No Scores today</p>
				<span>{{ scores.description }}</span>
			</div>
			<div v-else class="row">
				<div class="col">
					...
					<div id="divTable" class="container col-md-2" style="margin-right:-75px">
						...
						<table id="boxscores" class="table table-borderless">
							<thead id="tableHead">
								<tr id="tableRow">
									<th width="180px">
										<div v-if="!score.status">
											<span>No Score.Status for Inning length</span>
										</div>
										<!-- Status of game - 'final', 'f/10', canceled, etc -->
										<div colspan="1" id="gameStatus" v-else-if="score.status.inning > 9">
					...
					<div id="divTable" class="container col-sm-2" style="margin-left:-10px">
						<table v-if="!score.status || !score.linescore">
							<td>No Score.Status or Linescore for box scores</td>
						</table>
						<!-- Innings for < 11 Innigs --> 
						<table v-else-if="score.status.inning < 11" id="boxscores" class="table table-borderless">
							<thead id="tableHead">
								<tr id="tableRow">
									<th></th>
					...
					<div v-if="score.status" id="divTable" class="container col-sm-1" style="margin-left: 50px">
						<!-- Summary --> 
						<table id="boxscores" class="table table-borderless">
							<thead id="tableHead">
								<tr id="tableRow">
					...
					<div id="divTable" class="container col-sm-2" style="margin-left:-5px">
						<table v-if="!score.winning_pitcher || !score.losing_pitcher">
							No Score Pitchers
						</table>
						<table v-else id="boxscores" class="table table-borderless">
							<tbody id="playerBox">
								<tr id="tableRow" height="55px">
					...
					<div id="divTable" class="container col-sm-2" style="margin-left:-45px">
						<table v-if="!score.home_runs"></table>
						<table v-else id="boxscores" class="table table-borderless">
							<tbody>
								<tr id="tableRow" height="55px">
									<td>
	...
	export default {
		...
		computed: {
			...
			updateStoreScoreboard () {
				...
				this.noScoreObj.rendered = false
				return store.state.score.dailyScores.game
			},
			updateNoScore () {
				if (this.noScoreObj.rendered == false) {
					this.noScoreObj.data = store.state.score.dailyScores
					this.noScoreObj.rendered = true
					console.log("udpateNoScore " + JSON.stringify(this.noScoreObj.data))
					return this.noScoreObj.data
				}
				console.log("updateNoScore was already rendered")
			},
			updateTeamInfo () {
				this.teamInfo = store.state.team.teamArray 
			},
